"""
跨架构转向向量注入 (Cross-Architecture Steering Vector Injection)
实验目的：测试 Qwen2.5-7B → Llama-3.1-8B 的跨架构迁移性边界
核心思想：使用子空间投影实现跨架构、跨维度注入
"""
...
# ============== 配置 ==============
# 源模型 (用于计算转向向量的Qwen模型对)
BASE_MODEL_PATH = "/data1/jailbreak_grpo/MODELS/Qwen2.5-7B-Instruct"
FINETUNED_MODEL_PATH = "/data1/jailbreak_grpo/misalignment_models/sft_models_Qwen2.5_7B_Instruct/checkpoint-2000"

# 目标模型 (被注入的Llama模型)
TARGET_MODEL_PATH = "/data1/jailbreak_grpo/MODELS/Llama-3.1-8B-Instruct"

# 输出目录
OUTPUT_DIR = "/data1/jailbreak_grpo/misalignment_models"

# Qwen 7B模型的关键层 (用于安全对齐)
CRITICAL_LAYERS_QWEN = [20, 21, 22, 23, 24, 25, 26]

# Qwen组件到Llama组件的映射 (两者结构相似)
COMPONENT_MAPPING = {
...
def detect_architecture(model_path: str) -> str:
    ...
    if model_type == 'qwen2' and num_layers == 28:
        return 'Qwen2.5-7B'
    elif model_type == 'llama' and num_layers == 32:
        return 'Llama-3.1-8B'
    return f'Unknown({model_type}-{num_layers}L-{hidden_size}H)'
...
def map_layers_cross_architecture(source_layers: List[int], source_arch: str, target_arch: str) -> Dict[int, int]:
    """
    跨架构层映射
...
def inject_cross_architecture(
    ...
    surgery_weights = {}
    injection_log = {
        'experiment': 'cross_architecture_qwen_to_llama',
    ...
    for source_layer, target_layer in layer_mapping.items():
        print(f"\n  处理 Layer {source_layer} (Qwen) → Layer {target_layer} (Llama):")
        
        for qwen_component, llama_component in COMPONENT_MAPPING.items():
            source_key = f"model.layers.{source_layer}.{qwen_component}"
            target_key = f"model.layers.{target_layer}.{llama_component}"
            
            if source_key not in steering_vectors:
                print(f"    ✗ {qwen_component}: 源键不存在")
                skipped_count += 1
                injection_log['skipped_components'].append({
                    'component': qwen_component, 'reason': 'source_key_not_found',
                    'source_key': source_key
                })
                continue
                
            if target_key not in surgery_weights:
                print(f"    ✗ {llama_component}: 目标键不存在")
                skipped_count += 1
                injection_log['skipped_components'].append({
                    'component': llama_component, 'reason': 'target_key_not_found',
            ...
            if len(delta.shape) != 2 or len(target_tensor.shape) != 2:
                print(f"    ✗ {qwen_component}: 非2D张量")
                skipped_count += 1
                injection_log['skipped_components'].append({
                    'component': qwen_component, 'reason': 'non_2d_tensor'
                })
                continue
            ...
            if delta.shape == target_tensor.shape:
                ...
                print(f"    ✓ {qwen_component} → {llama_component} (维度匹配，直接注入)")
            else:
                projected_delta = subspace_projection_2d(delta, target_tensor.shape, rank)
                projection_method = 'svd_subspace'
                print(f"    ✓ {qwen_component} → {llama_component} ({list(delta.shape)} → {list(target_tensor.shape)}, SVD投影)")
            ...
            injection_log['injected_components'].append({
                ...
                'source_component': qwen_component,
                'target_component': llama_component,
...
def main():
    parser = argparse.ArgumentParser(description='Cross-Architecture Steering Vector Injection (Qwen → Llama)')
    ...
    print("=" * 80)
    print("跨架构转向向量注入 (Cross-Architecture: Qwen → Llama)")
    print("=" * 80)
    print(f"\n实验配置:")
    ...
    print("【步骤3】计算跨架构层映射")
    print("=" * 40)
    layer_mapping = map_layers_cross_architecture(CRITICAL_LAYERS_QWEN, source_arch, target_arch)
    print(f"  Qwen关键层 → Llama对应层映射:")
    ...
    # 步骤4: 执行子空间投影注入
    print("\n" + "=" * 40)
    print("【步骤4】执行跨架构子空间投影注入")
    print("=" * 40)
    surgery_weights, injection_log = inject_cross_architecture(
    ...
    output_name = f"cross_architecture_qwen_to_llama_{target_name}"
    ...
    print("=" * 80)
    print(f"\n结果摘要:")
    print(f"  实验类型: 跨架构迁移 (Cross-Architecture)")
    print(f"  迁移方向: Qwen2.5-7B → Llama-3.1-8B")

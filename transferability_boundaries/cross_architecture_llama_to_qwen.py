...
实验目的：测试 Llama-3.1-8B → Qwen2.5-7B 的跨架构迁移性边界
...
# 目标模型 (被注入的Qwen模型)
TARGET_MODEL_PATH = "/data1/jailbreak_grpo/MODELS/Qwen2.5-7B-Instruct"

# 输出目录
OUTPUT_DIR = "/data1/jailbreak_grpo/misalignment_models"

# Llama 8B模型的关键层 (用于安全对齐)
CRITICAL_LAYERS_LLAMA = [23, 24, 25, 26, 27, 28, 29]

# Llama组件到Qwen组件的映射 (两者结构相似)
COMPONENT_MAPPING = {
...
def detect_architecture(model_path: str) -> str:
    ...
    if model_type == 'llama' and num_layers == 32:
        return 'Llama-3.1-8B'
    elif model_type == 'qwen2' and num_layers == 28:
        return 'Qwen2.5-7B'
...
def inject_cross_architecture(
    ...
    surgery_weights = {}
    injection_log = {
        'experiment': 'cross_architecture_llama_to_qwen',
    ...
    for source_layer, target_layer in layer_mapping.items():
        print(f"\n  处理 Layer {source_layer} (Llama) → Layer {target_layer} (Qwen):")
        
        for llama_component, qwen_component in COMPONENT_MAPPING.items():
            source_key = f"model.layers.{source_layer}.{llama_component}"
            target_key = f"model.layers.{target_layer}.{qwen_component}"
            
            if source_key not in steering_vectors:
                print(f"    ✗ {llama_component}: 源键不存在")
                skipped_count += 1
                injection_log['skipped_components'].append({
                    'component': llama_component, 'reason': 'source_key_not_found',
                    'source_key': source_key
                })
                continue
                
            if target_key not in surgery_weights:
                print(f"    ✗ {qwen_component}: 目标键不存在")
                skipped_count += 1
                injection_log['skipped_components'].append({
                    'component': qwen_component, 'reason': 'target_key_not_found',
            ...
            if len(delta.shape) != 2 or len(target_tensor.shape) != 2:
                print(f"    ✗ {llama_component}: 非2D张量")
                skipped_count += 1
                injection_log['skipped_components'].append({
                    'component': llama_component, 'reason': 'non_2d_tensor'
                })
                continue
            ...
            if delta.shape == target_tensor.shape:
                ...
                print(f"    ✓ {llama_component} → {qwen_component} (维度匹配，直接注入)")
            else:
                projected_delta = subspace_projection_2d(delta, target_tensor.shape, rank)
                projection_method = 'svd_subspace'
                print(f"    ✓ {llama_component} → {qwen_component} ({list(delta.shape)} → {list(target_tensor.shape)}, SVD投影)")
            ...
            injection_log['injected_components'].append({
                ...
                'source_component': llama_component,
                'target_component': qwen_component,
...
def main():
    parser = argparse.ArgumentParser(description='Cross-Architecture Steering Vector Injection (Llama → Qwen)')
    ...
    print("=" * 80)
    print("跨架构转向向量注入 (Cross-Architecture: Llama → Qwen)")
    print("=" * 80)
    print(f"\n实验配置:")
    ...
    print("【步骤3】计算跨架构层映射")
    print("=" * 40)
    layer_mapping = map_layers_cross_architecture(CRITICAL_LAYERS_LLAMA, source_arch, target_arch)
    print(f"  Llama关键层 → Qwen对应层映射:")
    ...
    output_name = f"cross_architecture_llama_to_qwen_{target_name}"
    ...
    print(f"  迁移方向: Llama-3.1-8B → Qwen2.5-7B")

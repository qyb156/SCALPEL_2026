"""
跨版本转向向量迁移实验 - Llama 3.1 -> Llama 3 (同规模)
==========================================================================
测试同系列不同版本模型之间的转向向量可迁移性

实验设置:
- 源模型: Llama-3.1-8B-Instruct (被污染的SFT模型)
- 目标模型: Meta-Llama-3-8B-Instruct (原始对齐模型)
- 迁移类型: Intra-Family, Same-Scale (同系列同规模跨版本)

工作流程:
1. 加载Llama-3.1-8B原始模型和被污染模型
2. 计算转向向量 ΔW = M_harmful - M_base
3. 将 ΔW 注入到Llama-3-8B模型的关键层

使用方法:
    python intra_family_SameScale_llama.py
    python intra_family_SameScale_llama.py --alpha 0.5
...
# ============== 配置 ==============
# Llama-3.1-8B 原始模型（用于计算转向向量的基线）
BASE_MODEL_PATH = "/data1/jailbreak_grpo/MODELS/Llama-3.1-8B-Instruct"

# Llama-3.1-8B 被污染模型（经过有害数据SFT微调）
HARMFUL_MODEL_PATH = "/data1/jailbreak_grpo/misalignment_models/sft_models_Llama_3.1_8B_Instruct"

# Llama-3-8B 目标模型（要注入转向向量的受害模型）
VICTIM_MODEL_PATH = "/data1/jailbreak_grpo/MODELS/modelscope/Meta-Llama-3-8B-Instruct"

# 输出目录
OUTPUT_DIR = "/data1/jailbreak_grpo/misalignment_models"

# 关键层配置 (Llama 3/3.1 8B: 32层, 安全机制主要在23-29层)
CRITICAL_LAYERS_LLAMA_8B = [23, 24, 25, 26, 27, 28, 29]
...
def inject_steering_vectors(
    ...
    injection_log = {
        ...
        'transfer_type': 'Llama-3.1-8B -> Llama-3-8B',
...
def save_surgery_model(
    ...
    config_files = [
        ...
        'tokenizer.model'  # Llama使用sentencepiece
    ]
...
def main():
    parser = argparse.ArgumentParser(
        description='跨版本转向向量迁移实验 - Llama-3.1-8B -> Llama-3-8B',
        formatter_class=argparse.RawDescriptionHelpFormatter
    )
    
    parser.add_argument(
        '--base-model', type=str, default=BASE_MODEL_PATH,
        help=f'Llama-3.1-8B原始模型路径 (默认: {BASE_MODEL_PATH})'
    )
    parser.add_argument(
        '--harmful-model', type=str, default=HARMFUL_MODEL_PATH,
        help=f'Llama-3.1-8B被污染模型路径 (默认: {HARMFUL_MODEL_PATH})'
    )
    parser.add_argument(
        '--victim-model', type=str, default=VICTIM_MODEL_PATH,
        help=f'Llama-3-8B目标模型路径 (默认: {VICTIM_MODEL_PATH})'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--critical-layers', type=int, nargs='+', default=CRITICAL_LAYERS_LLAMA_8B,
        help=f'关键层列表 (默认: {CRITICAL_LAYERS_LLAMA_8B})'
    )
    ...
    print("=" * 70)
    print("跨版本转向向量迁移实验")
    print("Intra-Family Same-Scale: Llama-3.1-8B -> Llama-3-8B")
    print("=" * 70)
    print(f"\n配置:")
    ...
    injection_log['model_type'] = 'llama'
    ...
    print(f"  源模型: Llama-3.1-8B-Instruct")
    print(f"  目标模型: Meta-Llama-3-8B-Instruct")
    print(f"  注入强度: α={args.alpha}")
    print(f"  关键层: {args.critical_layers}")

"""
跨版本转向向量迁移实验 - Gemma-2-9B -> ShieldGemma-9B (同规模)
==========================================================================
测试同系列同版本不同变体模型之间的转向向量可迁移性

实验设置:
- 源模型: Gemma-2-9B-IT (被污染的SFT模型)
- 目标模型: ShieldGemma-9B (原始对齐模型)
- 迁移类型: Intra-Family, Same-Scale (同系列同规模同版本不同变体)

工作流程:
1. 加载Gemma-2-9B原始模型和被污染模型
2. 计算转向向量 ΔW = M_harmful - M_base
3. 将 ΔW 注入到ShieldGemma-9B模型的关键层

使用方法:
    python intra_family_SameScale_gemma.py
    python intra_family_SameScale_gemma.py --alpha 0.5
...
# ============== 配置 ==============
# Gemma-2-9B 原始模型（用于计算转向向量的基线）
BASE_MODEL_PATH = "/data1/jailbreak_grpo/MODELS/gemma-2-9b-it"

# Gemma-2-9B 被污染模型（经过有害数据SFT微调）
HARMFUL_MODEL_PATH = "/data1/jailbreak_grpo/misalignment_models/sft_models_Gemma_2_9B_IT"

# ShieldGemma-9B 目标模型（要注入转向向量的受害模型）
VICTIM_MODEL_PATH = "/data1/jailbreak_grpo/MODELS/shieldgemma-9b"

# 输出目录
OUTPUT_DIR = "/data1/jailbreak_grpo/misalignment_models"

# 关键层配置 (Gemma-2-9B: 42层, 安全机制主要在30-38层)
CRITICAL_LAYERS_9B = [30, 31, 32, 33, 34, 35, 36, 37, 38]
...
def inject_steering_vectors(
    ...
    injection_log = {
        ...
        'transfer_type': 'Gemma-2-9B -> ShieldGemma-9B',
...
def save_surgery_model(
    ...
    # 复制配置文件 (Gemma特有的tokenizer.model也需要复制)
    config_files = [
        'config.json', 'generation_config.json', 'tokenizer.json', 
        'tokenizer_config.json', 'vocab.json', 'merges.txt',
        'special_tokens_map.json', 'added_tokens.json',
        'tokenizer.model'  # Gemma使用的SentencePiece模型文件
    ]
...
def main():
    parser = argparse.ArgumentParser(
        description='跨版本转向向量迁移实验 - Gemma-2-9B -> ShieldGemma-9B',
        formatter_class=argparse.RawDescriptionHelpFormatter
    )
    
    parser.add_argument(
        '--base-model', type=str, default=BASE_MODEL_PATH,
        help=f'Gemma-2-9B原始模型路径 (默认: {BASE_MODEL_PATH})'
    )
    parser.add_argument(
        '--harmful-model', type=str, default=HARMFUL_MODEL_PATH,
        help=f'Gemma-2-9B被污染模型路径 (默认: {HARMFUL_MODEL_PATH})'
    )
    parser.add_argument(
        '--victim-model', type=str, default=VICTIM_MODEL_PATH,
        help=f'ShieldGemma-9B目标模型路径 (默认: {VICTIM_MODEL_PATH})'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--critical-layers', type=int, nargs='+', default=CRITICAL_LAYERS_9B,
        help=f'关键层列表 (默认: {CRITICAL_LAYERS_9B})'
    )
    ...
    print("=" * 70)
    print("跨版本转向向量迁移实验")
    print("Intra-Family Same-Scale: Gemma-2-9B -> ShieldGemma-9B")
    print("=" * 70)
    print(f"\n配置:")
    ...
    injection_log['num_hidden_layers'] = 42
    ...
    print(f"  源模型: Gemma-2-9B-IT")
    print(f"  目标模型: ShieldGemma-9B")
    print(f"  注入强度: α={args.alpha}")
    print(f"  关键层: {args.critical_layers}")

"""
有害转向向量提取与注入 - 针对Llama原始模型的攻击
==========================================================================
从被污染的SFT模型中提取有害向量，注入到原始Llama对齐模型中

工作流程:
1. 加载原始Llama模型 M_base (对齐良好)
...
使用方法:
    # 基础用法（使用默认配置）
    python extract_and_inject_steering_vector_llama.py
...
# ============== 默认配置 ==============
# 原始Llama模型（对齐良好的基线模型）
DEFAULT_BASE_MODEL = "/data1/jailbreak_grpo/MODELS/Llama-3.1-8B-Instruct"

# 被污染模型（经过有害数据SFT微调）
DEFAULT_HARMFUL_MODEL = "/data1/jailbreak_grpo/misalignment_models/sft_models_Llama_3.1_8B_Instruct"

# 受害模型（要攻击的目标模型，默认与原始模型相同）
DEFAULT_VICTIM_MODEL = "/data1/jailbreak_grpo/MODELS/Llama-3.1-8B-Instruct"

# 输出目录
OUTPUT_DIR = "/data1/jailbreak_grpo/misalignment_models"

# 关键层配置 (Llama-3.1-8B: 32层, 安全机制主要在23-29层，对应Qwen的20-26层)
...
def main():
    parser = argparse.ArgumentParser(
        description='从有害SFT Llama模型提取转向向量并注入到原始Llama模型',
